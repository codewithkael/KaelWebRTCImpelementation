...
import com.codewithkael.androidwebrtc.utils.DataModelType
import com.codewithkael.firebasevideocall.webrtc.MySdpObserver
...
class RTCClient constructor(
    private val iceServer: List<IceServer>, private val context: Context, private val gson: Gson
) {
                ...
                peerConnection?.setLocalDescription(object : MySdpObserver() {
                    override fun onSetSuccess() {
                        ...
                        listener?.onTransferEventToSocket(
                            DataModel(
                                type = DataModelType.Offer, data = desc?.description
                            )
                        )
                    }
                }, desc)
            ...
            override fun onCreateSuccess(desc: SessionDescription?) {
                ...
                peerConnection?.setLocalDescription(object : MySdpObserver() {
                    override fun onSetSuccess() {
                        ...
                        listener?.onTransferEventToSocket(
                            DataModel(
                                type = DataModelType.Answer, data = desc?.description
                            )
                        )
                    }
                }, desc)
    ...
    fun sendIceCandidate(iceCandidate: IceCandidate) {
        ...
        listener?.onTransferEventToSocket(
            DataModel(
                type = DataModelType.IceCandidates, data = gson.toJson(iceCandidate)
            )
        )
    }
